...
# Aim of script    : To calculate the indicator number of heat stress days (Index) (Fontes unpublished) in model 
# and obs. 
...
#################################################################################
## What is the Number of heat stress days (Index) ?
# "Number of heat stress days" is defined as annual count of days when daily maximum temperature exceeds 35 degC.
## In this script instead of using 35 degC, I am using its corresponding percentile to avoid bias in model.
...
source('/esarchive/scratch/pjha/home_cp_power9/others/functions/R_PlotMostLikelyQuantileMap.R')
...
lat<-ncvar_get(tmx_mod, "lat") 
lon<-ncvar_get(tmx_mod, "lon") 
...
## (1) step 1
## Convert 35 degC to the corresposndnig percentile in obs for each day from the (daily value of that day over the entire period (1993-2017 ## i.e. from 25 values)
obs_tmx35<-list()
for (m in 1:month){
obs_tmx35[[m]]<-array(numeric(), dim=c(length(lon), length(lat),days[m]))
for (i in 1:length(lon)) {
 for (j in 1:length(lat)) {
  for (k in 1:days[[m]]) {
obs_tmx35[[m]][i,j,k]<-ecdf(aa[[m]][-exclude,k,j,i])(35)    
    }
   }
 }
 }
 
## Compare the percentile position of each day's value with the threshold value calculated in step (1)  
## which days are greater than obs_tmx35

grt<-list()
for (m in 1:month){
grt[[m]]<-array(numeric(), dim=c(length(lon), length(lat),year,month,days[m]))
for (i in 1:length(lon)) {
  for (j in 1:length(lat)) {
    for (t in 1:year) {
     for (n in 1:days[[m]]) {
...
## total number of days in each year greater than 35 degC
Index<-array(numeric(), dim=c(length(lon), length(lat), year, month))
for (m in 1:month){
for (i in 1:length(lon)) {
  for (j in 1:length(lat)) {
    for (t in 1:year) {
     for (n in 1:days[[m]]) {
    Index[i,j,t,m]=sum(grt[[m]][i,j,t,m, ])
    }
}
}
}
...
################################## (2) forecasts
## the method is same as obs except an additional dimension, number of members 
### 
month=7
...
## which days in each year are greater than tmx 35 degC
grtr <- function(var_obs, var_exp)
{

grt1<-array(numeric(), c(n,t))
for (n in 1:nmemb) {
 for (t in 1:year) {
  if (ecdf(var_exp[n, ])(var_exp[n,t])>var_obs) { 
grt1[n,t]=1 
} else {
grt1[n,t]=0
}
...
grt_fc<- list()
Index_fc<-array(numeric(), dim=c(length(lon), length(lat), nmemb, year, month))

for (m in 1:month){
var_obs<-aperm(obs_tmx35[[m+5]], c(3,2,1))
names(dim(var_obs))<-c('ftime','lat','lon')
names(dim(frc[[m]]))<-c('nmemb','year','ftime','lat','lon')

grt_fc[[m]]<-Apply(
data=list(var_obs=var_obs,var_exp=frc[[m]]),
margins=list(c(1,2,3),c(3,4,5)), ncores = 15, fun = 'grtr')
}

## sum total days in each month
Index_fc<-array(numeric(), dim=c(nmemb, year,month,length(lat),length(lon)))
...
for (i in 1:length(lon)) {
  for (j in 1:length(lat)) {
    for(n in 1:nmemb) {
      for (t in 1:year) {
    Index_fc[n,t,m,j,i]=sum(grt_fc[[m]]$output1[n,t, ,j,i])
    }
}
}
}
...
### season sum
Index_seasTot_frc<-apply(Index_fc,c(1,2,4,5),sum)
...
## RPSS is computed for forecast season only
Index_obs_seas<-Index[ , , ,6:12]
...
frc11<-aperm(Index_seasTot_frc, c(4,3,2,1))

v1<-frc11[ , ,-exclude, ] 

# obs exclude year 2002 
...
raw_Index4<-aperm(frc11,c(4,3,2,1)) ## raw Index
