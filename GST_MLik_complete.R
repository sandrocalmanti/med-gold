...
ref<-'JRA-55'
startMonth<-'04'  
yearini<-'1993' ## modify start year according to your data
yearend<-'2017'  ## modify end year according to your data
...
## bind tmx and tmin
temp_obs<-abind(tmxobs.dat, tminobs.dat, along=5)
temp_mod<-abind(tmxfrc.dat, tminfrc.dat, along=6)

## average of tmx and tmin for each month
av_temp_obs<-Mean1Dim(temp_obs,5)
av_temp_frc<-Mean1Dim(temp_mod,6)

## Growing season temp (Av of Apr-Oct)
Index_obs1<-Mean1Dim(av_temp_obs,2)-273
Index_mod1<-Mean1Dim(av_temp_frc,3)-273

rm(av_temp_obs); rm(av_temp_frc)
...
names(dim(v1))<-c('lon', 'lat', 'sdate', 'member')
...
## New code for naming dim in the case of subsetting
PlotMostLikelyQuantileMap(Subset(probability,along='type',indices=c(m),drop='selected'),lon=lon, lat=lat, toptitle=toptitle1[i],
bar_titles=c("Below normal (%)","Normal (%)","Above normal (%)"),
intylat = 1, intxlon = 1, width=10, height=10, mask=mask_comb[ , ,i], col_mask='white', colNA='white')
dev.off()
